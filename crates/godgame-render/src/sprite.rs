...
use godgame_data::mobs::{MobSpecArt, MobSpecSeq, MobSpecSeqMode, MobSpecSeqState};
use godgame_data::sprites::{
...
///
/// A module constant and not a `godgame-core/src/config` export: it describes ONE
...
/// `&'static str` in `godgame-data`, and the rasteriser reads each exactly once.
...
///
/// `None` is a first-class answer. `godgame_core::items::ITEM_ICONS` already
...
/// Rasterise and upload every record in `godgame_data::sprites::SPRITES`.
///
...
/// bridge, supplying the three from `godgame-core` — see [`MobAtlases`].
///
...
mod tests {
    use super::*;
    use godgame_core::items::ITEM_ICONS;
    use godgame_data::sprites::sprite;
    ...
    fn mob_art_bakes_through_the_same_bridge_as_the_sprite_table() {
        ...
        let mut seen = 0;
        for spec in godgame_data::mobs::MOBS.iter() {
