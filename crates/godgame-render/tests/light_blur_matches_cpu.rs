...
use godgame_core::config::{CHUNK_CELLS, View, WINDOW_COLS, WINDOW_ROWS};
...
impl Harness {
    ...
    fn new() -> Option<Harness> {
        ...
        let (device, queue) = block_on(adapter.request_device(&wgpu::DeviceDescriptor {
            label: Some("godgame light blur parity"),
            ..Default::default()
        }))
        .ok()?;
...
fn quantising_the_field_before_the_blur_costs_under_a_byte() {
    ...
    let shadow = godgame_render::light::shadow_tint(1.0, 0.0);
