...
//! justify adding one. [`Heightmap`](yugen_core::sim::worldgen::heightmap::Heightmap)
...
use yugen_core::config::{CELL_SIZE, LIGHT_DOWNSCALE, SEED, View, WINDOW_COLS, WINDOW_ROWS};
use yugen_core::sim::grid::CellGrid;
use yugen_core::sim::materials::{CellId, block};
use yugen_data::sprites::SPRITES;

use yugen_render::cells::{CellShades, paint_cells};
use yugen_render::light::{
    EmitterScan, LightFrame, LightGrid, bake_vignette, grid_size, scan_emitters, vignette_size,
};
use yugen_render::particles::{MAX_PARTICLES, ParticleSystem};
use yugen_render::sprite::{BakedSprite, FromContentOpts, sprite_art_from_content};

/// Scene seed. Local to this file; see `yugen-core`'s `sim` bench for why a
...
/// by `yugen-core`'s `worldgen` bench, not invented again here.
...
/// [`SpriteAtlases`](yugen_render::sprite::SpriteAtlases) derives `Clone` and
