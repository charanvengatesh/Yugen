...
use godgame_core::config::{CHUNK_CELLS, WINDOW_COLS, WINDOW_ROWS};
use godgame_core::sim::grid::CellGrid;
use godgame_core::sim::materials::MAT_COUNT;
use godgame_core::sim::worldgen::ChunkGen;
use godgame_render::cellmap::{CellShadeParams, MATERIAL_SLOTS, build_params, new_shade_texture};
use godgame_render::cells::{
...
fn shader_source() -> String {
    ...
    assert_eq!(
        directives,
        vec!["#define_import_path godgame::cells"],
...
impl Harness {
    ...
    fn new() -> Option<Harness> {
        ...
        let (device, queue) = block_on(adapter.request_device(&wgpu::DeviceDescriptor {
            label: Some("godgame cell parity"),
            ..Default::default()
        }))
        .ok()?;
