    ...
    Schema {
        ...
        fields: vec![
            ...
            (
                ...
                Field::new("float")
                    .doc(
                        "How reluctantly a liquid SHEARS, 0..1 (liquids only). 0 -- \
                         water -- moves every tick it can and pays nothing for this \
                         field existing. Above 0: free fall is untouched (honey off a \
                         spoon drops as fast as water), but every deforming move -- \
                         sinking through a lighter liquid, toppling onto a diagonal, \
                         levelling sideways -- is skipped with this probability \
                         (staying AWAKE, so it oozes rather than freezes), and the \
                         sideways reach shrinks by the same fraction. A viscous liquid \
                         falls at full speed, lands in a heap, and levels slowly.",
                    )
                    .default_float(0.0)
                    .check(|v| {
