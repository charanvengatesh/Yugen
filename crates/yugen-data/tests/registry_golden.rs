...
//!
//! ```text
//! YUGEN_BLESS=1 cargo test -p yugen-data --test registry_golden
//! ```
//!
...
fn every_baselined_id_kept_its_code() {
    ...
    for (module, id, code) in yugen_data::all_codes() {
...
fn the_baseline_records_are_an_unmoved_prefix() {
    ...
    let base = baseline();
    let codes = yugen_data::all_codes();
...
fn every_baselined_table_slot_still_reads_the_same() {
    ...
    let base = baseline();
    let tables = yugen_data::all_tables();
...
fn every_baselined_matrix_bit_still_reads_the_same() {
    ...
    let base = baseline();
    let mats = yugen_data::all_matrices();
...
/// `YUGEN_BLESS` says so, and every comparison above returns early in that
...
fn bless() {
    ...
    for (module, name, table) in yugen_data::all_tables() {
    ...
    for (module, name, side, bits) in yugen_data::all_matrices() {
        let n = side * side;
        let set: Vec<J> = (0..n)
