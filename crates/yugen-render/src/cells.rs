...
//! and the pass below is pure computation over `yugen-core` types — no `App`,
//! no window, no GPU, and nothing that would stop it living in `yugen-core`
//! today.
//!
//! IT DOES NOT, AND THAT COSTS SOMETHING. `tests/cells_golden.rs` is a test
//! target of `yugen-render`, and `yugen-render` depends on `bevy` — so the
...
//! `yugen-core`'s worldgen and noise suites do get: they never link a renderer.
//!
//! The fix is to move this module into `yugen-core`, and it HAS NOT BEEN DONE.
...
///
/// Deliberately NOT `Noise::hash2` from `yugen-core`: that one is salted by a
