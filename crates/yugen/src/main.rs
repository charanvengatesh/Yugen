...
//!
//! Everything of substance is in `yugen-render`'s plugins (see
//! [`yugen_render`]). What is left here is the shell:
//!
...
//!     [`yugen_render::player::NoPlayer`].
//!
//! The free camera used to live here. It is input, so it moved to
//! [`yugen_render::input`] along with the mouse and the brush; what is left is
...
use yugen_core::config::{PLAYER_H, PLAYER_W};
use yugen_core::input::KEYS;
use yugen_core::script::Script;
use yugen_core::sim::edits::EditMode;
use yugen_core::sim::materials::{EMPTY, code_of};
use yugen_render::YugenRenderPlugin;
use yugen_render::daynight::{DayNight, WorldClock};
use yugen_render::dump::{DumpState, dump_when_ready};
use yugen_render::input::{CursorOverride, PlayerIntent, key_code};
use yugen_render::items::GroundItems;
use yugen_render::mobs::Creatures;
...
///
/// 60 Hz, matching `yugen_core::script::SCRIPT_HZ` — the language measures
...
fn main() -> AppExit {
    ...
    if args.world.is_some() || args.seed.is_some() {
        app.insert_resource(WorldSave {
            ...
            seed: args.seed.unwrap_or(yugen_core::config::SEED),
        });
    }
        ...
        match flag.as_str() {
            ...
            "--give" => {
                ...
                for one in spec.split(',') {
                    ...
                    let Some(code) = yugen_core::items::item_code_of(id.trim()) else {
...
fn usage() -> ! {
    eprintln!(
        "usage: yugen [--world DIR] [--seed N] [--at X,Y] [--time T] [--screenshot PATH]\n\
...
fn run_script(
    ...
    intent.0 = yugen_core::input::Intent::default();
...
/// [`Intent::for_substep`](yugen_core::input::Intent::for_substep) narrows to
