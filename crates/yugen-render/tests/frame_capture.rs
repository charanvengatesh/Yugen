...
//!
//! So this is the last link: it boots the real [`YugenRenderPlugin`] group
...
//! [`lowres`](yugen_render::lowres) points the world camera at. Everything —
...
use yugen_render::effects::Screenshake;
use yugen_render::input::PlayerIntent;
use yugen_render::player::PlayerBody;
use yugen_render::player_art::PlayerFigure;
...
/// [`DAY_LENGTH_S`](yugen_render::daynight::DAY_LENGTH_S) of 300 — the clock
...
/// luminance units.
///
/// At the clock's [`DEFAULT_START`](yugen_render::daynight::DEFAULT_START) of
...
fn capture_one_frame() -> Option<Capture> {
    ...
    let mut app = headless_game("Yūgen frame capture");
    app.finish();
    app.cleanup();
...
fn the_world_select_screen_is_reachable_and_paints() {
    ...
    app.insert_resource(yugen_render::worldselect::WorldPicker { root, ..default() });
    app.finish();
    app.cleanup();
...
fn the_light_follows_a_runtime_seed_rather_than_the_compiled_one() {
    ...
    let mut app = common::headless_game("Yūgen runtime seed");
    app.insert_resource(yugen_render::world::WorldSave {
        dir: None,
        seed: 777,
    });
    app.finish();
