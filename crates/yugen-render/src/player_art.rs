...
//! [`yugen_core::entities`]'s header, which is the other end of this: the
...
use yugen_core::config::{CELL_SIZE, PLAYER_CELLS_H, PLAYER_CELLS_W, PLAYER_H, PLAYER_W, scaled};
use yugen_core::entities::{AnimState, Player};
use yugen_data::sprites::{SPRITES, SpriteDef, SpriteSeqState, sprite};

// ---------------------------------------------------------------------------
// The tuning that came back with the drawing
// ---------------------------------------------------------------------------
//
// These are the numbers `yugen-core`'s entity header says it dropped and
...
///
/// [`AnimState`] is written by hand in `yugen-core` and [`SpriteSeqState`] is
...
/// [`yugen_core::entities`] publishes for exactly this purpose.
#[derive(Clone, Copy, Debug, PartialEq)]
